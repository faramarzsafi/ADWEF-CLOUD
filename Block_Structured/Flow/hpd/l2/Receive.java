...
public class Receive extends WorkflowBehaviour {
	...
	private void defineActivities() {
		...
		receiveSubflowActivity1Activity
				.setSubflow("Block_Structured.Flow.hpd.l2.Flow");
...
//		System.out.println("Receive: Executed"+share.recCount++);
		try {
			Thread.sleep(share.ACTIVITY_SLEEP);
		} catch (InterruptedException e) {e.printStackTrace();}
		
//		s.setPerformer("Flow_Agent");
		s.setPerformer(ContainerManagement.getContainer("Flow_Agent", "Block_Structured.Flow.hpd.l2.Flow"));
		s.fill("input", input);
