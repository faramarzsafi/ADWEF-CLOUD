...
public class OldWorkflowClient extends WorkflowBehaviour {
...
//		}
	...
	int perUnitRequestMode(long startTime, long speed, int rate, LinkedList threadList, long mili) throws Exception{

		PeriodicParameters pp = new PeriodicParameters(
			      new RelativeTime(mili, 0));
	    RTPeriodicClient rtc = new RTPeriodicClient(this, pp, threadList, startTime, speed, rate);
	    rtc.start();
	    rtc.join();
...
//			}
			...
			System.out.println("Statring...Rate: "+rate+" out of "+MAXRATE+" ,step="+RATE_STEP);
			System.out.println("Message Size = "+share.MESSAGE.length/1000+" kb");
...
//					nano = (int) ((sleepTime*1000000f-(double)mili*1000000f));
	...
	class RTPeriodicClient extends RealtimeThread {
		...
		Random random;
		int count = 0;
		OldWorkflowClient wfc;
		
		RTPeriodicClient(OldWorkflowClient wfc, PeriodicParameters pp, LinkedList threadList, long startTime, long speed, int rate){
		    super(null, pp);
	...
	class ClientThread extends Thread {
		...
		private int input;
		private int index;
		private OldWorkflowClient wc;
		
	
		public ClientThread(OldWorkflowClient wc, TupleSpace ts, int index, String SUBFLOW, String SUBFLOWAGENT){ 
			this.wc = wc;
			this.bag = ts;
			this.setStart(0);
			this.setStop(0);
