...
public class WorkflowClient_With_PoolSize extends WorkflowBehaviour {
...
//		}
	...
	int perUnitRequestMode(long startTime, long speed, int rate, LinkedList threadList, long mili) throws Exception{
	    ...
	    RTPeriodicClient rtc = new RTPeriodicClient(pp, threadList, startTime, speed, rate);
	    rtc.start();
	    rtc.join();
...
//					nano = (int) ((sleepTime*1000000f-(double)mili*1000000f));
	...
	class RTPeriodicClient extends RealtimeThread {
		...
		RTPeriodicClient(PeriodicParameters pp, LinkedList threadList, long startTime, long speed, int rate){
		    super(null, pp);
	...
	class ClientThread extends Thread {
		...
		private int input;
		private int index;
		private WorkflowClient_With_PoolSize wc;
		
	
		public ClientThread(WorkflowClient_With_PoolSize wc, TupleSpace ts, int index, String SUBFLOW, String SUBFLOWAGENT){ 
			this.wc = wc;
			this.bag = ts;
			this.setStart(0);
			this.setStop(0);
