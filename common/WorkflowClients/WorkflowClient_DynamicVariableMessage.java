...
public class WorkflowClient_DynamicVariableMessage extends WorkflowBehaviour {
...
//		FILE_PATH = share.FilePath(BULK, WORKFLOW_STYLE, NUMBER_OF_AGENTS, NUMBER_OF_SERVERS, SPEED, MAXRATE, RATE_START, RATE_STEP);
	...
	private void inlineThreadClient() {
		...
		File[] messages = {
		new File("/opt/temp/varMessages/50kb.xml"),
		new File("/opt/temp/varMessages/75kb.xml"),
		new File("/opt/temp/varMessages/100kb.xml"),
		new File("/opt/temp/varMessages/125kb.xml"),
		new File("/opt/temp/varMessages/150kb.xml"),
		new File("/opt/temp/varMessages/175kb.xml"),
		new File("/opt/temp/varMessages/200kb.xml"),
		};
		...
		try {
			...
			for (int vmCount = 0; vmCount< messages.length; vmCount++){
				
				Thread.sleep(share.DELAY_BETWEEN_EXPERIMENTS);// THIS IS TO MAKE A DELAY BETWEEN EXPs FROM MESSAGING POINT OF VIEW
				
				String fName = messages[vmCount].getName();
				String BASE_PATH = "/opt/temp/VarMesResults"+"/"+share.getWorkflowStyle(WORKFLOW_STYLE)//+ContainerManagement.numOfServers+"_"+ NUMBER_OF_AGENTS
					+"/"+rate+"/"+fName.substring(0, fName.indexOf('.')-1);
				String FILE_NAME ="/File"+"_"+BULK+"_"+share.getWorkflowStyle(WORKFLOW_STYLE)+"_"+ NUMBER_OF_AGENTS+"_"+ NUMBER_OF_SERVERS+"_"
					+SPEED+"_"+MAXRATE+"_"+ RATE_START+"_"+ RATE_STEP+".xml";
				FILE_PATH = BASE_PATH+FILE_NAME;
				File tvm = new File(FILE_PATH);
				(new File(tvm.getParent())).mkdirs();
				tvm.createNewFile();
				
				share.writeOutputHeader(new File(FILE_PATH));

				VariableMessage vm = new VariableMessage();
				Document varMes = vm.parseXmlFile(messages[vmCount].getPath(), false);
				...
				System.out.println("For Message File: "+fName );
...
//					rtn = bulkRequestMode(startTime, SPEED, rate, threadList);
					...
					throw new Exception("BULK messaging is inactvie");
				} else if (BULK == 2){
...
//		}
	...
	class RTPeriodicClient extends RealtimeThread {
		...
		int count = 0;
		Document vm;
		WorkflowClient_DynamicVariableMessage wfc;
		TupleSpace bag;
		Random random;
		
		RTPeriodicClient(WorkflowClient_DynamicVariableMessage wfc, PeriodicParameters pp, LinkedList threadList, long startTime, long speed, int rate, Document variableMessage){
		    super(null, pp);
	...
	class ClientThread extends Thread {
		...
		private int input;
		private int index;
		private WorkflowClient_DynamicVariableMessage wc;
	
		public ClientThread(WorkflowClient_DynamicVariableMessage wc, TupleSpace ts, int index, String SUBFLOW, String SUBFLOWAGENT){ 
			this.wc = wc;
			this.bag = ts;
			this.setStart(0);
			this.setStop(0);
